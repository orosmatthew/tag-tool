        ...
        arguments?.let{
            ...
            view?.findViewById<Button>(R.id.SaveTag)?.setOnClickListener {
                ...
                ServiceClient.updateTagType(tagId, newName).thenApply {res->
                    if(res)
                    {
                        val toast = Toast.makeText(requireContext(), "Tag Updated!", Toast.LENGTH_SHORT)
                        toast.show()
                    }

                    else
                    {
                        val toast = Toast.makeText(requireContext(), "Update Failed!", Toast.LENGTH_SHORT)
                        toast.show()
                    }
                }
            ...
            view?.findViewById<Button>(R.id.DeleteTag)?.setOnClickListener {
                ServiceClient.deleteTagType(tagId).thenApply { res->
                    ...
                    {
                        val toast = Toast.makeText(requireContext(), "Delete Failed!", Toast.LENGTH_SHORT)
                        toast.show()
                    }
                }
